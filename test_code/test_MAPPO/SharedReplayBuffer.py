...
def _flatten(T, N, x):
    return x.reshape(T * N, *x.reshape[2:])
...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.share_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents,
                                   *share_obs_shape), dtype=np.float32)
        self.obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents,
        ...
        if act_space.__class__.__name__ == 'Discrete':
            self.available_actions = np.ones((self.episode_length + 1, self.n_rollout_threads, num_agents,
                                              act_space.n), dtype=np.float32)
        else:
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
        ...
        self.actions[self.actions] = actions.copy()
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        self.returns[step] = (self.returns[step + 1] * self.gamma * self.masks[step + 1] +
                                             ...
                                             (1 - self.bad_masks[step + 1]) * value_normalizer.\
