...
class FootballRunner(Runner):
    ...
    def insert(self, data):
        ...
        rnn_states[dones_env==True] = np.zeros(((dones_env==True).sum(), self.num_agents, self.recurrent_N,
        ...
        for k, v in env_infos.items():
            if len(v) > 0:
                if self.use_wandb:
                    ...
                    self.writter.add_scalar(k, {k: np.mean(v)}, total_num_steps)
    ...
    def eval(self, total_num_steps):
        ...
        done_episodes_per_thread = np.zeors(self.n_eval_rollout_threads, dtype=int)
        ...
        if self.use_wandb:
            ...
            wandb.log({'eval_wi_rate': eval_win_rate}, step=total_num_steps)
    ...
    @torch.no_grad()
    def render(self):
        # render envs and init rnn and mask
        ...
        for i_episode in range(self.all_args.render_episodes):
            ...
            render_rnn_states = np.zeros((self.n_rollout_threads, self.num_agents, self.recurrent_N,
                                          self.hidden_size), dtype=np.float32)
