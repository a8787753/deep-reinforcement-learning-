...
def _shuffle_agent_grid(x, y):
    ...
    # col = np.stack([np.random.permutation(y) for _ in range(x)])
...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space, env_name):
        ...
        # will be used if obs_space.__class__.__name__ == 'list'
        ...
        # initialize bins(np.zeros) for data
    ...
    # useless for now
    def chooseinsert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
                     value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None):
        """
    ...
    def feed_forward_generator_transformer(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        :param advantages: (np.ndarray) advantages estimates.
