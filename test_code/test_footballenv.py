...
def _encode_ball_which_zone(ball_x, ball_y):
...
def _get_avail_new(obs, ball_distance):
...
ball_which_zone = _encode_ball_which_zone(ball_x, ball_y)
...
avail = _get_avail_new(obs, ball_distance)
...
obs_cat = np.hstack([np.array(state_dict[k], dtype=np.float32).flatten() for k in sorted(state_dict)])
ava = state_dict["avail"]
